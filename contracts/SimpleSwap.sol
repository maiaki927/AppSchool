...
contract SimpleSwap is ISimpleSwap, ERC20 {
    ...
    function swap(
        ...
        if (reserveA == 0 && reserveB == 0) {         
            ...
            amountOut = tokenIn==AToken? amountIn * reserveB / (reserveA+amountIn) : amountIn * reserveA / (reserveB+amountIn);          
            updateReserves(tokenIn==AToken? reserveA+amountIn:reserveA - amountOut,tokenIn==BToken? reserveB+amountIn:reserveB - amountOut);
        }  
    ...
    function getReserves() external override view returns (uint256 _reserveA, uint256 _reserveB){
        _reserveA=reserveA;
        _reserveB=reserveB;
